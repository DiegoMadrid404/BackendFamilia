...
namespace Proyecto.IC.Utilidades.Auxiliares.Comunes
{
    ...
    public static class ExtensionesBasicas
    {
        ...
        /// <summary>
        /// Normalizacion del texto sin acentos ni Ñ, reemplaza caracteres no alfanumericas por
        /// cadena vacia
        /// </summary>
        /// <param name="cadena">Cadena a normalizar</param>
        /// <returns>Una cadena normalizada</returns>
        public static string NormalizarTexto(this string cadena)
        {
            string resultado = string.Empty;

            if (string.IsNullOrEmpty(cadena) == false)
            {
                resultado = cadena.Normalize(NormalizationForm.FormD);

                Regex expresion = new Regex(rcsExpresionesRegulares.Alfanumerico);

                return expresion.Replace(resultado, string.Empty);
            }
